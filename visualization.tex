...
{\tt tabplot} is a R package for a visualization of a big data with various type of variables.
 ...
 In this method, the big data is summarized by the binning and plotted with this summarized information. With several options, the user can change the pre-fixed reference variable,
 zoom in the specific area, or use the subset of data.

\item scagnostics and ScagExplorer

http://cran.r-project.org/web/packages/scagnostics/index.html

 ScagExplorer: Exploring Scatterplots by Their Scagnostics - Dang, T.N., Wilkinson, L.
{\it Pacific Visualization Symposium (PacificVis), 2014 IEEE}
...
\item measuring and visualizing the stability of biomarker selection techniques

\item visualizing high density clusters in multidimensional data using optimized star coordinates

\item visual software analytics for the build optimization of large-scale software systems

\item linked treemap - a 3D treemap-nodelink layout for visualizing hierarchical structures

\item graphs as navigational infrastructure for high dimensional data spaces

\item Eulerian tour algorithms for data visualization and the PairViz package

\item a graphical goodness-to-fit test for dependence models in higher dimensions

\item visualization of categorical response models - from data glyphs to parameter glyphs

\item visualizing multiple quantile plots

\item the generalized ridge trace plot - visualizing bias and prediction

\item tradeoffs in information graphics

\item partition maps

\item determining and depicting relationships among components in high-dimensional variable selection

\item dissimilarity plots - a visual exploration tool for partitional clustering

\item multiclass data exploration using space transformed visualization plots

\item pairwise display of high dimensional information via Eulerian tours and hamiltonian decompositions
